...
export interface EastMoneyInstrumentPriceProviderOptions {
  fetchFn?: FetchLike;
  endpoint?: string;
}
...
export class EastMoneyInstrumentPriceProvider implements IInstrumentPriceProvider {
  readonly name = "Eastmoney";

  private readonly fetchFn: FetchLike;
  private readonly endpoint: string;

  constructor(options: EastMoneyInstrumentPriceProviderOptions = {}) {
  ...
  async fetchLatestPrices(input: FetchLatestInstrumentPricesInput): Promise<InstrumentPriceProviderResult> {
    ...
    for (const instrument of input.instruments) {
      ...
      prices.push(parseEastMoneyResponse(await response.json(), instrument.sourceSymbol, instrument.currency));
    }
...
function parseEastMoneyResponse(
  ...
  return {
    sourceSymbol,
    priceDate: timestampToIsoDate(row.f86, sourceSymbol),
    closePrice: requiredPositiveDecimal(row.f43, `Eastmoney close price for ${sourceSymbol}`),
