...
`.env.test` is ignored and used for deployed test configuration. `.env.test.example` documents the expected keys. The real `.env.test` should point `VITE_API_BASE_URL` at `https://test-fund-api.kidrawer.com`, not the local API adapter.
