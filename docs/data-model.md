...
Portfolio snapshots are durable but repairable daily valuation records generated by scheduled jobs or manual backfills. The app business day uses a `06:00 Asia/Shanghai` cutoff so snapshot dates are stable across viewer time zones and before China/Hong Kong markets open.
...
- Dashboard trend charts can request sampled trend points from the snapshot API. Portfolio value sampling is daily for `近1月`, weekly for `近3月`, and monthly for `近1年` or longer. Weekly/monthly target dates align to the current app business date; non-trading targets use the latest previous snapshot while keeping the target date as the chart label.
