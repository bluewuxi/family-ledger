...
- `portfolio_snapshot_headers`: stored snapshot identity, date, historical FX rates, notes, and timestamps.
- `portfolio_snapshots`: read-only aggregate view derived from snapshot headers and account snapshots.
...
- Dashboard trend charts can request trend points from the snapshot API. Portfolio value normally uses every stored daily snapshot in the selected range. For long ranges of two years or more (`近3年`, `近5年`, or `投资以来`), the API thins portfolio points to weekly targets aligned to the current app business date. Principal-event dates resolve against the full snapshot history, not the sampled chart. A carried snapshot is unavailable if any principal event occurred after its date and through the target date; this prevents deposits/withdrawals without a matching valuation from appearing as profit/loss. With no intervening principal event, targets can carry the latest previous snapshot and retain its actual `snapshotDate`.
- The profit chart shows the change in `资产净值 - 总投入` since its first available point in the selected range. Missing valuations remain gaps, and decorative live-curve midpoints are excluded from profit calculations. Cumulative profit uses the latest actual snapshot/live value, never the chart's principal/zero fallback; it is unavailable when the latest value is missing.
- A live valuation without an adjacent live value is drawn as a visible marker; missing valuation gaps are not connected to fabricate a trend.
...
All API, job, and repair snapshot writes use the service-role-only `upsert_portfolio_snapshot(jsonb)` function. After cutover it serializes writes for each date, upserts the header, and atomically replaces the complete account set, including an empty set. Failure rolls back both. Aggregate input fields remain accepted for compatibility but are not stored. The all-account view derives USD totals and warning multisets; purpose-filtered history aggregates matching account rows through the shared calculation. Closing a security position to zero resets its remaining cost and cost-availability flags, so a reopened position does not inherit missing historical FX from a closed position.

## Monthly Family Review

The monthly family review combines a derived API/UI report with a small persisted workflow record. Derived financial values are not duplicated into monthly report rows. The app stores only one `monthly_reviews` row per non-future `YYYY-MM` month for family notes and review completion metadata.

The report reuses stored portfolio snapshots for month-start and month-end values, manual principal transactions for net invested cash, manual cash adjustments for month-end reconciliation, and dividend transactions as API-compatible income data. The web monthly review intentionally does not render a separate dividend section; dividend generated cash affects snapshots and cash balances and therefore remains part of the residual valuation movement. Buy/sell generated cash legs are shown only as linked settlement context under their parent trade activity; dividend generated cash legs are not returned as linked rows in the monthly summary DTO.

Monthly bridges require a snapshot exactly on the day before the month and on the closing date. Completed months close at calendar month-end; the current month closes at the last completed app business date, with transactions limited to that same date. An older boundary snapshot retains its date but has unavailable portfolio/account values and a `SNAPSHOT_DATE_MISMATCH` warning; it is not treated as initialization. Missing boundary dates must be rebuilt before the report can calculate complete profit. Price publication lag within a correctly dated snapshot remains allowed.

For correctly dated snapshots, each account uses its own valuation availability. An unavailable portfolio total caused by one account does not hide other accounts' known values or gains; portfolio-dependent contribution percentages remain unavailable.

For the initialization month, when no earlier portfolio snapshot exists, manual `opening_balance` transactions and market-valued `opening_position` transactions are treated as a synthetic month-start baseline at both portfolio and account level. Opening balances use their cash amount. Opening positions use quantity, the latest available instrument price on or before the opening date, and valuation FX; `gross_amount` remains carrying cost and is not used as market value. Opening rows are not counted again as in-month net principal flow for the bridge or its `净投入明细`, so `资产变化` and `估值变动` remain meaningful without double-counting opening assets. If a security opening lacks price or FX data, the synthetic baseline is unavailable and the report surfaces a data-quality warning.

Account changes compare account snapshot rows from the selected start and end snapshots, then subtract account-level principal flow and cash adjustments to derive account valuation movement. Contribution percentage divides account valuation movement by the sum of absolute account-level valuation movements, so in-month principal changes do not inflate contribution shares. A missing account row on one side is displayed as zero so accounts opened or closed during the month remain explainable. Rows with unavailable snapshot values or missing account-level FX remain unavailable and surface data-quality warnings rather than partial values.
